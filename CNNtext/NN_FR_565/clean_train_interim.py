...
parser.add_argument('--wrd_classes', default=565,
...
def train(mode = FLAGS.mode, restore_path=None, save_path=FLAGS.save_path, plot=0, show=0):
    ...
    if mode == 'pre':
        ...
        cat_scores = np.load(FLAGS.save_path + 'cat_scores_pre_z_full_nomir.npy')
        ...
        shift_epoch = 55
        max_epochs = FLAGS.max_epochs_pre - shift_epoch
        print_save = 'saving pre-schooler model'

        # Model
        net = clean_cornets.CORnet_Z_tweak()
        checkpoint = torch.load(FLAGS.save_path + 'save_pre_z_'+str(shift_epoch)+'_full_nomir.pth.tar')['state_dict']
