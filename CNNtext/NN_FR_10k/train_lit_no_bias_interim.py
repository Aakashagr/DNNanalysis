...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  1 12:57:34 2022
...
def train_nobias(mode = FLAGS.mode, restore_path=None, save_path=FLAGS.save_path, plot=0, show=0):
    ...
    if 'lit' in mode:
        ...
        max_epochs = FLAGS.max_epochs_lit - nepoch_tr
        cat_scores = np.zeros((FLAGS.max_epochs_pre + FLAGS.max_epochs_lit, classes))
        cat_scores = np.load(FLAGS.save_path + 'cat_scores_lit_no_bias_z_full_nomir.npy')
        print('np.shape(cat_scores)', np.shape(cat_scores))
        trainloss, valloss = np.load(FLAGS.save_path + 'trainloss_lit_no_bias_z_full_nomir.npy').tolist(), np.load(
            FLAGS.save_path + 'valloss_lit_no_bias_z_full_nomir.npy').tolist()

        shift_epoch = FLAGS.max_epochs_pre + nepoch_tr

        # Model

        checkpoint = torch.load(FLAGS.save_path + 'save_lit_no_bias_z_62_full_nomir.pth.tar')['state_dict']
        model_dict =["V1.conv.weight", "V1.conv.bias", "V2.conv.weight", "V2.conv.bias", "V4.conv.weight",
 				 "V4.conv.bias", "IT.conv.weight", "IT.conv.bias", "linear.weight", "linear.bias"]
        ...
        with torch.set_grad_enabled(False):
            ...
            for local_batch_val, local_labels_val in validation_gen:
                ...
                if cat_index < 565:
                    ...
                    for r in range(10):
                        scores = Acc(pred_val[r*5:(r+1)*5], local_labels_val[r*5:(r+1)*5])
                        print('category accuracy scores', scores)
                        cat_scores[shift_epoch + epoch, cat_index] = scores
                        print('cat_scores[shift_epoch + epoch, cat_index]', cat_scores[shift_epoch + epoch, cat_index])
                        # exec_time = secondsToStr(time.time() - start_time)
