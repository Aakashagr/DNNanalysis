...
parser.add_argument('--img_path', default='wordsets_EN_CH',
                    help='path to ImageNet folder that contains train and val folders')
parser.add_argument('--wrd_path', default= 'wordsets_EN_CH',
...
parser.add_argument('--num_train_items', default=13,
                    help='number of training items in each category')
parser.add_argument('--num_val_items', default=5,
...
#             ckpt_data = torch.load(FLAGS.save_path + 'save_pre_z_49_full_nomir.pth.tar')['state_dict']
#             ########################################## SPECIFIC TO local
#             for key in list(ckpt_data.keys()):
#                 if 'module.' in key:
#                     ckpt_data[key.replace('module.', '')] = ckpt_data[key]
#                     del ckpt_data[key]
#                 #############################################
...
#             net_pre.load_state_dict(ckpt_data['state_dict'])
            net_pre.load_state_dict(ckpt_data['state_dict'], strict=False)
