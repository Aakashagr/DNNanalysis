...
""" provides 'sublexstats' objects that tracks the frequency of letters, bigrams, trigrams and words from lists """
...
class sublexstats:
    ...
    def import_lexfreqs(df):
        """ df is a dataframe with two columns: type, freq """
        words = df.iloc[:, 0]
        weights = df.iloc[:, 1]
        for wd, we in zip(words, weights):
            self.add(unidecode(wd), we)
...
if __name__ == '__main__':
    import pprint as pp
    mydic = sublexstats()
    mydic.import_csv('french-lexique-reduced.tsv')

    # two examples:
    print("bonjour : ")
    pp.pprint(mydic.get_all_stats('bonjour'))
    print()
    print("aliata : ")
    pp.pprint(mydic.get_all_stats('aliata'))