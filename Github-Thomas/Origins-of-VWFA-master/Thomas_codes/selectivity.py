...
def warp_process_store(epoch=79, mode='lit_bias', clip=False):
    ...
    paths = ['faces/', 'bodies/', 'houses/', 'tools/', 'false_fonts/', 'infreq_letters/', 'freq_letters/',
             'freq_bigrams/', 'freq_quadrigrams/', 'words/']
...
def analyze_units(target_cat='words', baseline_cats=['faces', 'bodies', 'houses', 'tools'], mode='lit_bias', epoch=79,
                  crit=3, verbose=False, show=False):
    dim = 512
    n_ex = 100
    n_cat = 10
    categories = ['faces', 'bodies', 'houses', 'tools', 'false_fonts', 'infreq_letters', 'freq_letters', 'freq_bigrams',
                  'freq_quadrigrams', 'words']
    ...
    for unit in range(dim):
        codes[:, :, unit] = codes[:, :, unit] / np.max(
            codes[:, :, unit])  # normalize by the maximum unit response over all categories
...
def word_grabs2(means, mode='no_bias', show=True, save=True, verbose=True):
    if mode == 'pre':
        suptext = 'Initial tuning of word-selective units \n (n =' + str(
            int(np.sum(means))) + ' units, pre-literate model)'
    if mode == 'lit_no_bias':
        suptext = 'Initial tuning of word-selective units \n (n =' + str(
            int(np.sum(means))) + ' units, literate model without bias)'
    if mode == 'lit_bias':
        suptext = 'Initial tuning of word-selective units \n (n =' + str(
            int(np.sum(means))) + ' units, literate model with bias)'
    if mode == 'illit':
        suptext = 'Initial tuning of word-selective units \n (n =' + str(
            int(np.sum(means))) + ' units, illiterate model)'
    ...
    categories = ['faces', 'bodies', 'houses', 'tools', 'false_fonts', 'infreq_letters', 'freq_letters', 'freq_bigrams',
                  'freq_quadrigrams', 'words', 'uncommited']
    readable = ['faces', 'bodies', 'houses', 'tools', 'false fonts', 'frequent letters', 'infrequent letters',
                'bigrams', 'quadrigrams', 'words', 'labile']
    legendary = ['faces', 'bodies', 'houses', 'tools', 'false fonts', 'letters \n(low F)', 'letters \n(high F)',
                 'bigrams', 'quadrigrams', 'words', 'labile']
    colors = ['red', 'violet', 'yellow', 'blue', 'aquamarine', 'mediumaquamarine', 'lightgreen', 'lime', 'forestgreen',
              'darkgreen', 'gray']
...
def plot_all_dense_unit_responses(mode='lit_no_bias', epoch=79, save=True, show=True):
    ...
    ranges = [range(64), range(64, 128), range(128, 192), range(192, 256), range(256, 320), range(320, 384),
              range(384, 448), range(448, 512)]
    ...
    for units in ranges:
        ...
        gs0 = gridspec.GridSpec(2, 1, figure=f, height_ratios=[1, 20], left=0.06, bottom=0.10, right=0.94, top=0.90,
        ...
        for i in range(n_cat):
            ...
            rect = patches.Rectangle((xshift + ratio, yshift), 0.01, 1, linewidth=15, edgecolor=colors[i],
                                     facecolor=colors[i])
        ...
        if mode == 'bias':
            ...
            ratiot = (i + 1) / 12.0
            # Create a Rectangle patch
            rect = patches.Rectangle((xshift + ratio, yshift), 0.01, 20, linewidth=5, edgecolor='red',
                                     facecolor=colors[i], fill=False)
        ...
        for unit in units:
            ...
            codes[:, :, unit] = codes[:, :, unit] / np.max(
                codes[:, :, unit])  # normalize by the maximum unit response over all categories
            data = [codes[0, :, unit], codes[1, :, unit], codes[2, :, unit], codes[3, :, unit], codes[4, :, unit],
                    codes[5, :, unit], codes[6, :, unit], codes[7, :, unit], codes[8, :, unit], codes[9, :, unit]]
            ...
            lg2 = plt.legend([leg1], ['unit ' + str(unit)], handlelength=0, loc=2, ncol=1, prop={'size': 10},
                             framealpha=0, bbox_to_anchor=(-0.15, 1.15))
            lg2.get_frame().set_linewidth(0)

        # add 
        # save figure
        if save:
            plt.savefig('profiles_' + mode + '_' + str(epoch) + '_' + strings[cf] + '.png', dpi=300,
                        bbox_inches='tight')
        if show:
            plt.show()
        plt.clf()
    ...
    eyeballed_no_bias_string_selective_units = [4, 9, 29, 38, 41, 59, 83, 89, 92, 109, 110, 113, 115, 131, 142, 150,
                                                152, 161, 165, 166, 170, 194, 197, 203, 204, 210, 212, 222, 226, 229,
                                                232, 237, 242, 246, 250, 252, 254, 265, 270, 278, 282, 283, 291, 293,
                                                294, 302, 308, 311, 315, 318, 323, 351, 362, 367, 372, 377, 385, 389,
                                                392, 407, 410, 426, 432, 434, 436, 442, 451, 483, 487, 507]
