...
checkpoint = torch.load('save_rep1/save_lit_no_bias_z_77_full_nomir.pth.tar',map_location ='cpu')['state_dict']
...
for l in layer:
    ...
    print(['% of Telugu word-selective units in '+l+' :'+ str(np.size(neuid)/np.size(data_d,1)) + ', count: ' + str(np.size(neuid))])
